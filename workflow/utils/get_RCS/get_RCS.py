...
def get_RCS(chr_, pos1, pos2, strand, circ_id, up_type, down_type, tmp_dir='.'):
    # upstream
    up_bed = Bed6(chr_, max(pos1 - args.dist, 0), pos1, circ_id, '.', strand)
    up_bed_file = save_tmp(up_bed, dir_=tmp_dir)

    up_fa = get_fasta(up_bed_file.name, args.ref_file)
    up_fa_file = save_tmp(up_fa, dir_=tmp_dir)

    # downstream
    down_bed = Bed6(chr_, pos2 - 1, pos2 - 1 + args.dist, circ_id, '.', strand)
    down_bed_file = save_tmp(down_bed, dir_=tmp_dir)

    down_fa = get_fasta(down_bed_file.name, args.ref_file)
    down_fa_file = save_tmp(down_fa, dir_=tmp_dir)
